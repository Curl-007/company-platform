...
- `documents`: REST and WebSocket collaboration now share the same document-management policy and module boundary. Collaboration updates require an optimistic `baseRevision`, re-check the active user and document scope for every write, broadcast only committed updates, and retain revision-aware audit evidence.
...
1. Introduce a PostgreSQL Repository adapter behind the existing module contracts and validate export/import manifests before changing the production data source.
2. Continue shrinking `server.js` by moving remaining cross-domain helper functions behind existing module contracts.
3. Add shared/generated enum and API-contract sources once the module boundaries stabilize.

## Remaining High-Value Boundaries

- Backend: database connection adapters and remaining cross-domain helper functions in `server.js`.
- Frontend: remaining large presentation-only sections and shared/generated contracts.
