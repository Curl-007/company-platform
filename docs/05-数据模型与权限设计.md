...
> **文档基线**：本文同时保留**目标态模型**与**当前实际实现模型**。当前实现以 [§2.15 实际数据库 Schema（as-built）](#215-实际数据库-schemaas-built以此为准) 为准；目标态表或能力（如 `ai_analysis`、独立测试计划表、pgvector 语义检索）未实现时不应作为联调依据。差异台账见 [10 实现状态与差异清单](./10-实现状态与差异清单.md)。
>
> **表的阶段归属**：
...
| `document_chunk` | `id/document_id/project_id/chunk_index/section_title/page_no/content/content_hash/token_estimate/embedding_provider/embedding_model/embedding_vector/indexed_at` | 当前用于关键词 RAG 切片索引；embedding 字段预留，尚未接 pgvector |
| `rag_citation` | `id/query_hash/document_id/chunk_id/project_id/quote/score/source/created_by/created_at` | 记录 RAG 检索命中的可审计引用证据 |
...
- `org_units` 是组织单元规范表；`users.department_id` 关联组织单元，`users.department` 是受同步维护的兼容展示文本。组织写操作与成员归属变更均由管理员执行并记录审计。
- `users` 表已支持 CRUD（`GET/POST/PATCH/DELETE /api/users`，`admin:*` 权限），禁用用户鉴权拦截（登录/已登录请求/WebSocket 均 403）。个人资料不可自行变更部门归属。
- 任务已通过 `tasks.sprint_id` 与 Sprint 关联；项目接口会返回可追溯的迭代和任务关系。
