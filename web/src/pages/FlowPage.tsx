...
function FlowPage() {
  ...
  return (
    <div>
      ...
      <Panel title="流程模板（二选一）" subtitle="系统只提供固定交付与轻量交付，项目在详情页绑定其一" className="mt-12">
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 12 }}>
              {templates.map((template: WorkflowTemplate) => {
                ...
                return (
                  ...
                  >
                    {template.name}
                    <span style={{ marginLeft: 6, opacity: 0.8 }}>{template.stages?.length || 0} 阶段</span>
                  </button>
                );
              })}
            </div>
      ...
      <Panel title="项目阶段矩阵" subtitle="点击项目行展开详情；列顺序跟随上方选中的模板预览" className="mt-12">
    ...
    <div className="flow-detail">
      {data.workflow?.templateName && (
        <p className="text-secondary" style={{ marginTop: 0 }}>
          {"当前绑定：" + data.workflow.templateName + (data.workflow.templateVersion ? (" · " + data.workflow.templateVersion) : "")}
        </p>
      )}
