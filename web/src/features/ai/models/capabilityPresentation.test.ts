...
describe('external_action capabilities (browser-control)', () => {
  function browserControlManifest() {
    return {
      id: 'browser-control',
      version: '1.0.0',
      status: 'approved',
  ...
  it('accepts the external_action risk alongside read_only manifests', () => {
    const normalized = normalizeAiCapabilityManifest(browserControlManifest());
    expect(normalized).not.toBeNull();
    expect(normalized?.risk).toBe('external_action');
    expect(normalized?.inputSchema.properties).toHaveProperty('action');

    // Unknown risks stay rejected.
    expect(normalizeAiCapabilityManifest({
  ...
  it('renders screenshotKey entries as artifact links', () => {
    const manifest = normalizeAiCapabilityManifest(browserControlManifest());
    expect(manifest).not.toBeNull();

    const entries = capabilityOutputEntries(manifest!, {
      ok: true,
      action: 'open',
      screenshotKey: 'AIC-123',
      text: 'hello',
    });
    const screenshot = entries.find((entry) => entry.name === 'screenshotKey');
    expect(screenshot?.value).toBe('AIC-123');
    expect(screenshot?.link).toBe('/api/ai/browser/screenshots/AIC-123');

    // Non-artifact fields carry no link.
    const text = entries.find((entry) => entry.name === 'text');
    expect(text?.link).toBeUndefined();

    // Empty screenshot keys render as plain values without a link.
    const empty = capabilityOutputEntries(manifest!, { ok: true, action: 'open', screenshotKey: '' });
    expect(empty.find((entry) => entry.name === 'screenshotKey')?.link).toBeUndefined();
  });
});
