...
import type { AiChatAttachment, AiChatMessage, AiJob } from '../../../types';
import {
  MAX_ATTACHMENTS,
  buildReviewDraft,
...
export default function AiView() {
  ...
  const [messages, setMessages] = useState<AiChatMessage[]>([
    createWelcomeMessage('我是项目管理 AI 助手。你可以直接问项目进度、风险、需求、缺陷、交付，也可以上传截图或文档让我分析。'),
  ]);
  ...
  function resetChat() {
    setMessages([
      createWelcomeMessage('新的对话已开始。可以问我项目现状，也可以上传截图、需求文档或测试记录。', `welcome-${Date.now()}`),
    ]);
    setAttachments([]);
    setDraft('');
    setFileError(null);
