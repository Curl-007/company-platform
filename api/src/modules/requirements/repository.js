function createRequirementsRepository({ insert, row, rows, run }) {
  ...
  return {
    ...
    findPortfolio: (id) => row("SELECT id, product_ids FROM portfolios WHERE id = @id", { id }),
    listChildren: (parentId) => rows("SELECT * FROM requirements WHERE parent_id = @pid AND deleted_at IS NULL ORDER BY id", { pid: parentId }),
    listActiveProjectIdsByProduct: (productId) => rows(
      "SELECT id FROM projects WHERE product_id = @productId AND deleted_at IS NULL ORDER BY id",
      { productId },
    ),
    listRequirements,
    ...
    updateRequirement: (next) => run(
      `UPDATE requirements SET title=@title, description=@description, priority=@priority,
       acceptance_criteria=@acceptanceCriteria, product_id=@productId, portfolio_id=@portfolioId,
       parent_id=@parentId, assignee=@assignee,
