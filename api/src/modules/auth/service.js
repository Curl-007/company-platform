...
function createAuthService({ comparePassword, issueToken, publicUser, repository }) {
  return {
    ...
    updateProfile(userId, input) {
      const before = repository.findById(userId);
      ...
      if (repository.findOtherByEmail(profile.email, userId)) throw authError("CONFLICT", "该邮箱已被其他用户使用。", 409);
      const after = repository.updateProfile(userId, profile);
      return { before, after };
    },
  };
}
