...
function createProjectsRouter({
  ...
  router.get("/projects", (req, res) => {
    const allItems = repository.listProjects(req.query)
      .filter((project) => canAccessProject(req.user, project.id))
      .map(mapProject);
    res.json(ok(paginatedResponse(allItems, req.query)));
  });

  router.post("/projects", requirePermission("project:*"), (req, res) => {
    ...
    const idempotency = beginIdempotentRequest(req, res, "project.create");
    ...
    try {
      const response = transaction(() => {
        const project = buildProjectCreate(
          ...
          { id: nextId("PRJ", "projects"), now: now(), json },
        );
        repository.createProject(project);
        statusHistory.record({ resourceType: "project", resourceId: project.id, projectId: project.id, toStatus: project.status, reason: "项目创建", actor: req.user });
        audit(req.user, "project.create", "project", project.id, null, project, req.ip);
        const created = ok(mapProject(repository.findProject(project.id)));
  ...
  router.get("/projects/:id/status-history", (req, res) => {
    const project = repository.findProjectId(req.params.id);
    if (!project) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canAccessProject(req.user, project.id)) return fail(res, 403, "PERMISSION_DENIED", "无权访问该项目状态历史。");
    res.json(ok(statusHistory.list("project", project.id)));
  });

  router.patch("/projects/:id/status", requirePermission("project:*"), (req, res) => {
    ...
    const before = repository.findProject(req.params.id);
    if (!before) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canManageProject(req.user, before.id)) return fail(res, 403, "PERMISSION_DENIED", "无权变更该项目状态。");
    ...
    if (req.body.status === "active" && before.status !== "active") {
      const readiness = projectActivationReadiness(before);
    ...
    const updateResult = repository.updateProjectStatus({ id: req.params.id, status: req.body.status, updatedAt: now(), expectedVersion });
    if (updateResult.changes === 0) {
      const current = repository.findProjectVersion(req.params.id);
    ...
    const after = repository.findProject(req.params.id);
    statusHistory.record({ resourceType: "project", resourceId: after.id, projectId: after.id, fromStatus: before.status, toStatus: after.status, reason: req.body?.statusReason, actor: req.user });
    audit(req.user, "project.status_update", "project", req.params.id, before, after, req.ip);
    res.json(ok(mapProject(after)));
  });

  router.patch("/projects/:id", requirePermission("project:*"), (req, res) => {
    const before = repository.findProject(req.params.id);
    if (!before) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canManageProject(req.user, before.id)) return fail(res, 403, "PERMISSION_DENIED", "无权编辑该项目。");
    const expectedVersion = expectedProjectVersion(req, res, before);
    if (expectedVersion === null) return;
    ...
    if (status !== undefined) {
      ...
      if (status === "active" && before.status !== "active") {
        const readiness = projectActivationReadiness(next);
        if (!readiness.ok) return fail(res, 409, "PROJECT_ACTIVATION_GATE_BLOCKED", "Project needs an objective, planned dates, members, a milestone or sprint, approved capacity allocations, and matching capacity plans before activation.", readiness);
      }
    }
    const updateResult = repository.updateProject(next);
    if (updateResult.changes === 0) {
      const current = repository.findProjectVersion(req.params.id);
      return fail(res, 409, "VERSION_CONFLICT", "Project was changed by another user. Refresh and retry your update.", { expectedVersion, currentVersion: Number(current?.version) || null });
    }
    const after = repository.findProject(req.params.id);
    if (status !== undefined) statusHistory.record({ resourceType: "project", resourceId: after.id, projectId: after.id, fromStatus: before.status, toStatus: after.status, reason: req.body?.statusReason, actor: req.user });
    audit(req.user, "project.update", "project", req.params.id, before, after, req.ip);
    res.json(ok(mapProject(after)));
  });

  router.delete("/projects/:id", requirePermission("project:*"), (req, res) => {
    const before = repository.findProject(req.params.id);
    if (!before) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canManageProject(req.user, before.id)) return fail(res, 403, "PERMISSION_DENIED", "无权删除该项目。");
    const dependencies = repository.projectDependencies(req.params.id);
    ...
    repository.softDeleteProject({ id: req.params.id, deletedAt: now() });
    audit(req.user, "project.delete", "project", req.params.id, before, null, req.ip);
    res.json(ok({ deleted: true, id: req.params.id }));
  });

  router.get("/projects/:id", (req, res) => {
    const project = repository.findProject(req.params.id);
    if (!project) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canAccessProject(req.user, project.id)) return fail(res, 403, "PERMISSION_DENIED", "无权访问该项目。");
    const mapped = mapProject(project);
    const projectTasks = repository.listProjectTasks(req.params.id).map(mapTask);
    const projectSprints = repository.listProjectSprints(req.params.id).map(mapSprint);
    res.json(ok({ ...mapped, tasks: projectTasks, sprints: projectSprints }));
  });

  router.get("/projects/:id/sources", requirePermission("source:read"), (req, res) => {
    const project = repository.findProject(req.params.id);
    if (!project) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canAccessProject(req.user, project.id)) return fail(res, 403, "PERMISSION_DENIED", "Cannot access sources for this project.");
    try {
  ...
  router.post("/projects/:id/milestones", requirePermission("project:*"), (req, res) => {
    const before = repository.findProject(req.params.id);
    if (!before) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canManageProject(req.user, before.id)) return fail(res, 403, "PROJECT_ARCHIVED_OR_ACCESS_DENIED", "Cannot add milestones to an archived or inaccessible project.");
    ...
    repository.updateProjectMilestones({ id: before.id, milestones: json(milestones), updatedAt: now() });
    const after = repository.findProject(before.id);
    audit(req.user, "project.milestone_add", "project", before.id, before, after, req.ip);
    return res.status(201).json(ok({ milestones }));
  });

  router.delete("/projects/:id/milestones/:index", requirePermission("project:*"), (req, res) => {
    const before = repository.findProject(req.params.id);
    if (!before) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canManageProject(req.user, before.id)) return fail(res, 403, "PROJECT_ARCHIVED_OR_ACCESS_DENIED", "Cannot remove milestones from an archived or inaccessible project.");
    ...
    repository.updateProjectMilestones({ id: before.id, milestones: json(milestones), updatedAt: now() });
    const after = repository.findProject(before.id);
    audit(req.user, "project.milestone_remove", "project", before.id, before, after, req.ip);
  ...
  router.get("/projects/:id/members", (req, res) => {
    const project = repository.findProject(req.params.id);
    if (!project) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canAccessProject(req.user, project.id)) return fail(res, 403, "PERMISSION_DENIED", "无权访问该项目成员。");
    const members = repository.listProjectMembers(req.params.id).map(mapMember);
    res.json(ok(members));
  });

  router.post("/projects/:id/members", requirePermission("project:*"), (req, res) => {
    const project = repository.findProject(req.params.id);
    if (!project) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    if (!canManageProject(req.user, project.id)) return fail(res, 403, "PERMISSION_DENIED", "无权管理该项目成员。");
    ...
    const user = repository.findActiveUserByName(userName);
    if (!user) return fail(res, 400, "VALIDATION_FAILED", "userName must identify an active user.");
    const existing = repository.findProjectMemberByIdentity({ projectId: req.params.id, userId: user.id, userName, role });
    if (existing) {
      if (!existing.user_id) repository.reconnectProjectMemberUser(existing.id, user.id);
    ...
    repository.saveProjectMember(member);
    audit(req.user, "project.member_add", "project", req.params.id, null, member, req.ip);
    res.status(201).json(ok(mapMember(member)));
  });

  router.delete("/projects/:id/members/:memberId", requirePermission("project:*"), (req, res) => {
    if (!canManageProject(req.user, req.params.id)) return fail(res, 403, "PERMISSION_DENIED", "无权管理该项目成员。");
    const member = repository.findProjectMember(req.params.memberId, req.params.id);
    if (!member) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project member not found.");
    repository.deleteProjectMember(req.params.memberId);
