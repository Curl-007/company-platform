function createAiAdviceService({
  ...
  const providerName = () => (typeof getProviderName === "function" ? getProviderName() : "local-rule-engine");

  async function createRequirementRecommendation(score) {
    const requirement = row("SELECT * FROM requirements WHERE id = @id AND deleted_at IS NULL", { id: score.requirementId });
  ...
  async function createBusinessAdvice({ targetType, targetId, question, draft }) {
    const context = businessAdviceContextService.load(targetType, targetId);
    ...
    return businessAdviceHelpers.normalizeBusinessAdvicePayload(
      parsed ? { ...parsed, modelUsed: modelName(), generatedBy: providerName(), fallback: false } : fallback,
      fallback,
    );
  }
