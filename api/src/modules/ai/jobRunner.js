...
function createDocumentAnalysisRunner({ repository, analyzeDocument, json, now }) {
  ...
  async function run(jobId, document) {
    let job = await repository.findJob(jobId);
    if (!job) throw new Error("AI job not found.");
    try {
      job = await repository.transition(job, "running", {
        progress: 30,
        current_step: "文档解析中",
        started_at: now(),
      });
      ...
      await repository.transition(job, "awaiting_review", {
        progress: 100,
      ...
      await repository.updateDocumentAiStatus(document.id, "awaiting_review");
      return { status: "awaiting_review", result, evidence };
    } catch (error) {
      const current = await repository.findJob(jobId);
