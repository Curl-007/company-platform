...
function createCapacityRouter({
  ...
  function workloadThresholds() {
    const setting = repository.getSetting(thresholdSettingKey);
  ...
  router.get("/capacity/overview", (req, res) => {
    if (!canViewCapacity(req.user)) return fail(res, 403, "PERMISSION_DENIED", "只有项目经理和管理员可以查看团队容量。");
    res.json(ok(buildOverview(resolvePeriod(req.query), undefined, workloadThresholds())));
  });

  router.get("/capacity/me", (req, res) => {
    const overview = buildOverview(resolvePeriod(req.query), req.user.id, workloadThresholds());
    res.json(ok(overview.members[0] || null));
  });

  router.get("/capacity/settings/workload-thresholds", (req, res) => {
    if (!canViewCapacity(req.user)) return fail(res, 403, "PERMISSION_DENIED", "只有项目经理和管理员可以查看容量阈值。");
    res.json(ok(workloadThresholds()));
  });

  router.put("/capacity/settings/workload-thresholds", requirePermission("admin:*"), (req, res) => {
    ...
    const before = workloadThresholds();
    repository.saveSetting(thresholdSettingKey, JSON.stringify(thresholds), now());
    audit(req.user, "capacity.workload_thresholds_update", "app_setting", thresholdSettingKey, before, thresholds, req.ip);
    res.json(ok(thresholds));
  });

  router.get("/capacity/calendar", (req, res) => {
    ...
    res.json(ok({ period, ...mapCalendar(resolveCalendar(period)) }));
  });

  router.put("/capacity/calendar", requirePermission("project:*"), (req, res) => {
    ...
    const after = defaultCalendar();
    audit(req.user, "work_calendar.update", "work_calendar", calendar.id, before, after, req.ip);
    const period = resolvePeriod(body);
    res.json(ok({ period, ...mapCalendar(resolveCalendar(period)) }));
  });

  router.post("/capacity/calendar/exceptions", requirePermission("project:*"), (req, res) => {
    if (!canViewCapacity(req.user)) return fail(res, 403, "PERMISSION_DENIED", "You cannot maintain the work calendar.");
    const calendar = defaultCalendar();
    const body = req.body || {};
    ...
    const existing = repository.findCalendarException({ calendarId: calendar.id, date });
    const exception = { id: existing?.id || nextId("CALX", "work_calendar_exceptions"), calendar_id: calendar.id, calendar_date: date, is_working_day: body.isWorkingDay ? 1 : 0, name: String(body.name || "").trim(), created_at: existing?.created_at || now(), updated_at: now() };
    repository.upsertCalendarException(exception);
    audit(req.user, "work_calendar.exception_upsert", "work_calendar_exception", exception.id, existing || null, exception, req.ip);
  ...
  router.delete("/capacity/calendar/exceptions/:id", requirePermission("project:*"), (req, res) => {
    if (!canViewCapacity(req.user)) return fail(res, 403, "PERMISSION_DENIED", "You cannot maintain the work calendar.");
    const exception = repository.findCalendarExceptionById(req.params.id);
    if (!exception) return fail(res, 404, "RESOURCE_NOT_FOUND", "Calendar exception not found.");
    repository.deleteCalendarException(exception.id);
    audit(req.user, "work_calendar.exception_delete", "work_calendar_exception", exception.id, exception, null, req.ip);
    res.json(ok({ deleted: true, id: exception.id }));
  });

  router.put("/capacity/plans/:userId", requirePermission("project:*"), (req, res) => {
    if (!canViewCapacity(req.user)) return fail(res, 403, "PERMISSION_DENIED", "无权维护团队容量。");
    const user = repository.findActiveUser(req.params.userId);
    if (!user) return fail(res, 404, "RESOURCE_NOT_FOUND", "Active user not found.");
    const period = resolvePeriod(req.body || {});
    if (period.periodEnd < period.periodStart) return fail(res, 400, "VALIDATION_FAILED", "periodEnd must not be before periodStart.");
    const existing = repository.findPlan({ userId: req.params.userId, ...period });
    ...
    const plan = {
      id: existing?.id || nextId("CAP", "capacity_plans"),
    ...
    const mapped = mapPlan(repository.findPlanById(plan.id), calendar.workingDays);
    audit(req.user, "capacity.plan_upsert", "capacity_plan", plan.id, before, mapped, req.ip);
    res.json(ok(mapped));
  });

  router.put("/capacity/allocations", requirePermission("project:*"), (req, res) => {
    ...
    if (!canManageProject(req.user, projectId)) return fail(res, 403, "PERMISSION_DENIED", "无权维护该项目的资源投入。");
    const user = repository.findActiveUser(userId);
    if (!user) return fail(res, 404, "RESOURCE_NOT_FOUND", "Active user not found.");
    const project = repository.findProject(projectId);
    if (!project) return fail(res, 404, "RESOURCE_NOT_FOUND", "Project not found.");
    const membership = repository.findProjectMembership({ projectId, userId, userName: user.name });
    ...
    const existing = repository.findAllocationForProject({ projectId, userId, ...period });
    const plan = repository.findPlan({ userId, ...period });
    const calendar = resolveCalendar(period);
    const effectiveHours = mapPlan(plan, calendar.workingDays)?.effectiveHours || 0;
    const currentAllocations = repository.listOtherAllocations({ userId, ...period, existingId: existing?.id || "" });
    ...
    const allocation = {
      id: existing?.id || nextId("ALLOC", "project_allocations"),
    ...
    repository.upsertAllocation(allocation);
    const mapped = mapAllocation(allocation, project, effectiveHours);
    audit(req.user, needsOverrideApproval ? "capacity.allocation_override_submitted" : "capacity.allocation_upsert", "project_allocation", allocation.id, existing || null, mapped, req.ip);
    res.json(ok(mapped));
  });

  router.patch("/capacity/allocations/:id/approval", requirePermission("admin:*"), (req, res) => {
    const allocation = repository.findAllocation(req.params.id);
    ...
    repository.approveAllocation({ id: allocation.id, approvedBy: req.user.id, approvedAt: now(), updatedAt: now() });
    const after = repository.findAllocation(allocation.id);
    const project = repository.findProject(after.project_id);
    const plan = repository.findPlan({ userId: after.user_id, periodStart: after.period_start, periodEnd: after.period_end });
    const effectiveHours = mapPlan(plan, resolveCalendar({ periodStart: after.period_start, periodEnd: after.period_end }).workingDays)?.effectiveHours || 0;
    const mapped = mapAllocation(after, project, effectiveHours);
    audit(req.user, "capacity.allocation_override_approved", "project_allocation", after.id, allocation, mapped, req.ip);
    res.json(ok(mapped));
  });

  router.delete("/capacity/allocations/:id", requirePermission("project:*"), (req, res) => {
    if (!canViewCapacity(req.user)) return fail(res, 403, "PERMISSION_DENIED", "无权维护项目投入。");
    const allocation = repository.findAllocation(req.params.id);
    if (!allocation) return fail(res, 404, "RESOURCE_NOT_FOUND", "Allocation not found.");
    if (!canManageProject(req.user, allocation.project_id)) return fail(res, 403, "PERMISSION_DENIED", "无权维护该项目的资源投入。");
    repository.deleteAllocation(req.params.id);
    audit(req.user, "capacity.allocation_delete", "project_allocation", allocation.id, allocation, null, req.ip);
