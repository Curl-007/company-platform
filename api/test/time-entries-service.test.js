...
test("time entry service applies project scope and task ownership rules", async () => {
  const repository = {
  ...
  assert.equal((await service.validateProjectAndTask({ id: "USR-1" }, input)).ok, true);
  assert.equal((await service.validateProjectAndTask({ id: "USR-1" }, { ...input, taskId: "TASK-2" })).code, "VALIDATION_FAILED");
  assert.deepEqual(await service.listForUser({ id: "USR-1" }), [{
